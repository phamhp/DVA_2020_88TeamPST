var vizList = ["https://public.tableau.com/views/FlawedDiamonds/Dashboard1?:display_count=y&publish=yes&:origin=viz_share_link&:toolbar=no",
"https://public.tableau.com/views/FlawedDiamonds/Dashboard2?:display_count=y&publish=yes&:origin=viz_share_link&:toolbar=no"];
...
function getVizData(callback) {
  ...
  if (vizDisplay == 0) {
    ...
    sheet2 = viz.getWorkbook().getActiveSheet().getWorksheets().get("TopOR");
  }

	sheet2.getUnderlyingDataAsync(options).then(function (t) {
        if (vizDisplay == 0) {cleanData(t);} else {cleanData2(t)};
      callback();
    }); 
...
function cleanData2(table) {
	var columns = table.getColumns();
	var data = table.getData();	
  finalData = formatData(columns, data).sort(function(obj1, obj2) {
    return obj1.diff - obj2.diff || obj1.Stars - obj2.Stars;
  })
};
...
function getVizDataHG() {
  ...
  if (vizDisplay == 1) {
    sheet2 = viz.getWorkbook().getActiveSheet().getWorksheets().get("Overrated");
	...
	sheet2.getUnderlyingDataAsync(options).then(function (t) {
      ...
      clearList();
      document.getElementById('heading').innerHTML = "No Flawed Diamonds";
    } else {
      getVizData(update);
    }
  }); 
...
function Selection(n, id) {
  ...
  if (id_selected == n) {
    ...
    console.log("3");
    addr = finalData[n_new]['Address'];
    console.log(addr);
    city = finalData[n_new]['City'];
    state = finalData[n_new]['State'];
    zip = finalData[n_new]['Postal Code'];
    star = parseFloat(finalData[n_new]['Stars']).toFixed(2);
    orig_star = parseFloat(finalData[n_new]['Final Rating']).toFixed(2);
    diff = parseFloat(finalData[n_new]['diff']).toFixed(2) * -1;
    if (vizDisplay == 1) {overvalue = half_break + "<span style='font-weight:bold'>Traditional Rating: </span>" + orig_star + " Stars<br><span style='font-weight:bold'> Amount Overvalued: </span>" + diff + " Stars <br>" + half_break} else {overvalue = half_break} 
    document.getElementById(lookup).innerHTML = overvalue + "<span style='font-weight:bold'>Address:</span><br>" +  addr + ",<br>" + city.concat(", ", state, ", ", zip);
  }
}
...
function update() {
  ...
  if (vizDisplay == 1) { 
    document.getElementById('heading').innerHTML = "Flawed Diamonds";
  } else {
...
function changeFontColor(){
  if (vizDisplay == 1) {
    ...
    document.getElementById('heading').classList.remove("red");
  }
}
